...
class LeitorPDF:
    ...
    def text_pdf_fp(self, document: str, clean_text: bool, n_page: int = None, max_pages: int = 0) -> str:
        ...
        with open(document, 'rb') as f:
            extract_text_to_fp(
                f, output_string, maxpages=max_pages, page_numbers=n_page)
        return self._can_clean_text(clean_text, output_string.getvalue())


    def text_pdf_buff_fp(self, document: BufferedReader, clean_text: bool, n_page: int = None, max_pages: int = 0) -> str:
        output_string = StringIO()
        extract_text_to_fp(document, output_string, 
                           maxpages=max_pages, page_numbers=n_page)
        return self._can_clean_text(clean_text, output_string.getvalue())
